...
void Router::add_route(const uint32_t route_prefix,
    ...
    std::cerr << "DEBUG: adding route " << Address::from_ipv4_numeric(route_prefix).ip() << "/" << int(prefix_length)
         << " => " << (next_hop.has_value() ? next_hop->ip() : "(direct)") << " on interface " << interface_num << std::endl;

    _table.push_back(RouteEntry{route_prefix,
                                prefix_length,
                                next_hop,
                                interface_num
                               });
...
void Router::route_one_datagram(InternetDatagram &dgram) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (e.prefix_length < max_len) continue;
        uint32_t res = dst_ip_address ^ e.route_prefix;
        if (e.prefix_length == 0 || (res >> (32 - e.prefix_length)) == 0) {
            found_match = 1;
    ...
    interface(e.interface_num).send_datagram(dgram,
